...
public class Tasks {
    ...
    public static void getMapZone(){
        try {
            if(Variable.settings.getString(SharedKeys.deliveryAddress, "").equals("")){
    ...
    public static void getMapLottype(){
        try {
            if(Variable.settings.getString(SharedKeys.lotType, "").equals("")){
                ...
                JSONObject lottypeMap = data.getJSONObject("data").getJSONObject("auctionTypeMap");
